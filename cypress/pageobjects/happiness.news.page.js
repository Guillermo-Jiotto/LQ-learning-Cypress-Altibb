export class HappinessNewsPage {
    ...
    talkToDoctorBtn = () => cy.get('[data-event-name="Consultation Lead"][class*="in-read-cta-btn"]').first()
    askDoctorHeader = () => cy.get('[class="ask-question-title"]')

    watchAnswerBtn = () => cy.get('[data-event-name="Inread Question Clicked"]')
    answerQuestionsSection = () => cy.get('[class*="question-card-wrapper"]') // should be.visible
    answerQuestionHeader = () => cy.get('h1[class="question-heading"]') // should have.text
    ...
    askDoctorHeaderText = 'اطرح سؤالك الآن ليصل إلى آلاف الأطباء' // should include this text
    answerQuestionHeaderText = 'سؤال مهم ما هي اعراض فيروس كورونا الهندي و البريطاني معدة فم اقدام' // should have.text
    ...
    assertTalkToDoctorHeader = () => {
        this.talkToDoctorBtn().click({force: true})
        this.askDoctorHeader().then((el)=> {
            assert.include(el.text(), this.askDoctorHeaderText);
          });
    }

    assertWatchAnswerPage = () => {
        this.watchAnswerBtn().click({force: true})
        this.answerQuestionsSection().should('be.visible')
        this.answerQuestionHeader().should('have.text', this.answerQuestionHeaderText)
    }
